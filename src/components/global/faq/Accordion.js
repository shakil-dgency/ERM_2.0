  ...
  return (
    <li {...props}>
      <header
        ...
        className="relative flex justify-between items-center border-t-[1px] border-t-[#191D23] pb-[30px] lg:pb-[50px] pt-[25px] lg:pt-[45px] text-[20px] lg:text-[32px] font-[500] text-[#fff] cursor-pointer"
      >
        <span className="relative z-10 mt-1">{trigger}</span>
        <div
          className={`transition-transform absolute right-0 sm:right-4 ${open ? "rotate-180" : ""}`}
          aria-hidden
        >
          {/* simple chevron */}
      ...
      >
        <div className="lg:pt-[30px] pb-[50px]" ref={ref}>
          {children}
        </div>
      </div>
    </li>
